...
> rename to `.mcworld` to import it. The HTTP server (started with `serve`) packages the output
> automatically via the `/convert` family of endpoints, choosing the extension by edition:
> `.mcworld` for Bedrock (the default) and `.zip` for Java. The spawn point is derived from the
> input bbox (or set explicitly via `--spawn-x/y/z` / `--spawn-lat/lon`) and stored in `level.dat`
> as `SpawnX/Y/Z`. Java Edition output is a `.zip` of `region/` files plus `level.dat` and
> `session.lock` for use with the Java Edition client; see [CLI Reference](./CLI.md).
...
> **Warning:** This rename only works when the `.zip` actually contains a Bedrock world
