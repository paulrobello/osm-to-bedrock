...
```
osm_to_bedrock/
├── Cargo.toml           # par-osm-rust pinned to =0.5.0 (ARC-011)
...
> in the pinned external `par-osm-rust = "=0.5.0"` crate; the type and behavior descriptions
...
> from `par-osm-rust = "=0.5.0"`. The SRTM `.hgt` loader, bilinear interpolation, and the
...
> SHA-256-keyed disk cache all live in the pinned external `par-osm-rust = "=0.5.0"` crate.
...
> `par-osm-rust = "=0.5.0"`. `FeatureFilter` is defined upstream; edit there to extend it.
...
> `par-osm-rust = "=0.5.0"`. The `overturemaps` CLI wrapper, GeoJSON → OsmData
