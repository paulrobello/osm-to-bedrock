...
| `src/{osm,overpass,osm_cache,filter,elevation,srtm,overture}.rs` | **Thin re-export shims** from the pinned `par-osm-rust = "=0.5.0"` crate (ARC-011). The real parser / Overpass / cache / filter / elevation / SRTM / Overture logic lives there; edits to these 7 stub files are no-ops. Extension work belongs in `par-osm-rust`, not here. |
