...
**Partial:** `src/server/{auth,error,state,options}.rs` have unit tests for auth middleware, error mapping, job state, and options parsing, but no test drives real requests through the Axum router — upload/convert/poll/download and malformed-input paths are not yet covered end-to-end.
